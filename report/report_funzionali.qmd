...
Per ciascuno dei due criteri sopra menzionati si sono quindi introdotti i vincoli nel problema di ottimizzazione che può essere scritto come un programma di programmazione quadratica ([A1](#A1: Splines vincolate penalizzate)) per cui sono disponibili delle routine.
...
Da cui $d = y^\top \Phi$ e $D = \Phi^\top \Phi + \lambda P$.
Usando la definizione di scrittura in basi il vincolo è $0 \leq \phi^\top(f)b \leq 1 \quad \forall f$, in pratica si discretizza $\phi(f_j)$ per $j = 1,..,J$.
